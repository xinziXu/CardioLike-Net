...
def determine_r(q_on, s_off, ecg):
    ...
    for i in range(len(trend_dir)-1):
        if ((trend_dir[i] == 3) and (trend_dir[i+1] == 2)): # up_flat,flat_down
            if ecg[q_on + i] > r_temp:
                r_temp = ecg[q_on + i]
                r_loc_temp = q_on + i
            else:
    ...
    for i in range(len(trend_dir)-2):
        if ((trend_dir[i] == 3) and (trend_dir[i+1] == 0) and (trend_dir[i+2] == 2)):#up_flat,flat_flat,flat_down
            if ecg[q_on + i]> r_temp:
                r_temp = ecg[q_on + i]
                r_loc_temp = q_on + i
            else:
...
def determine_t(t_on, t_off,r, ecg):
    ...
    if len(trend_dir) > 5:
        ...
        for i in range(len(trend_dir)-1):
            if (trend_dir[i] == 5) or ((trend_dir[i] == 3) and (trend_dir[i+1] == 2)): #up_down/ up_flat, flat_down
                t_pos_temp = ecg[t_on + i]
                t_pos_loc_temp = t_on + i
            else:
                t_pos_temp = t_pos_temp 
...
def fiducial_points_vary(label,ecg):
    ...
    # for b in [25]:
        ...
        if len(qrs_info) != 0:
            for qrs_i in range(qrs_info.shape[0]):
                ...
                if (((106)<= qrs_info[qrs_i, 1]) and ((106)>= qrs_info[qrs_i, 0])): # 106 is within qrs wave

...
# label: (num_samples, 1)
    def __init__(self, ecg, fiducial_points, rrs): 
        if (len(ecg.shape) == 3):
            self.ecg = ecg[:,:,0]
        else:
            self.ecg = ecg
        if ecg.shape[0] == 1:
            ...
            self.ecg = np.expand_dims(self.ecg,0)
        else:
            self.ecg = np.squeeze(self.ecg)
...
#  utils
    ...
    def RR_AVE (self):
        ...
        return np.floor(sum(self.latest_8norm_rr) / len(self.latest_8norm_rr))
    ...
    def PR_AVE(self):
        return np.floor(sum(self.latest_8norm_pr) / len(self.latest_8norm_pr))
    ...
    def QRS_AVE(self):
        return np.floor(sum(self.latest_8norm_qrs) / len(self.latest_8norm_qrs))

    def R_AMP(self, id):
        if self.QRS_EXIST(id):
            # print(int(self.fiducial_points[id, 3]))
            # print(self.fiducial_points[id, 5])
            r_amp = self.ecg[id, int(self.fiducial_points[id, 5])]
    ...
    def R_AMP_AVE(self):
        return np.floor(sum(self.latest_8norm_r_amp) / len(self.latest_8norm_r_amp))
    ...
    def S_AMP_AVE(self):
        return np.floor(sum(self.latest_8norm_s_amp) / len(self.latest_8norm_s_amp))

    def Q_AMP(self, id):
        if self.QRS_EXIST(id):
            q_amp = np.floor(self.ecg[id, int(self.fiducial_points[id, 4])])
        else:
            q_amp = 0
        return q_amp

    def Q_AMP_AVE(self):
        return np.floor(sum(self.latest_8norm_q_amp) / len(self.latest_8norm_q_amp))
    ...
    def P_AMP_AVE(self):
        return np.floor(sum(self.latest_8norm_p_amp)/len(self.latest_8norm_p_amp))
    ...
    def T_AMP_AVE(self):
        return np.floor(sum(self.latest_8norm_t_amp)/len(self.latest_8norm_t_amp))
    ...
    def INIT_AVE(self):
        ...
        for i in range(0, self.ecg.shape[0]):
            ...
            if num_norm == 8:
                self.begin_id = i
                # print(self.begin_id)
                # print(self.latest_8norm_r_amp)
                # print('000000000000',np.floor(sum(self.latest_8norm_r_amp) / len(self.latest_8norm_r_amp)))
                break
    ...
    def RR_PRE_RR_AVE(self, id):
        if (self.begin_id <= id +1):
            ...
            #     print('self.RR_PRE(id)',id, self.RR_PRE(id),self.RR_AVE())
    ...
    def RR_POST_RR_AVE(self, id):
        if (self.begin_id <= id +1):
            # if id < 12:
            #     print(' self.RR_POST(id)',id, self.RR_POST(id),self.RR_AVE())
